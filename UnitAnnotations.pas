    ...
    try
      ...
      while Not(Eof(pNOT)) do begin
       try
        ...
        // coordonnées du texte
        AN.X               :=StrToFloatDef(LA[FA_COL_BASEPT_X],0.00);
        AN.Y               :=StrToFloatDef(LA[FA_COL_BASEPT_Y],0.00);
        AN.Z               :=StrToFloatDef(LA[FA_COL_BASEPT_Z],0.00);
        AN.dx              :=StrToFloatDef(LA[FA_COL_BASEPT_OF_X],0.00);
        AN.dy              :=StrToFloatDef(LA[FA_COL_BASEPT_OF_Y],0.00);
        AN.dz              :=StrToFloatDef(LA[FA_COL_BASEPT_OF_Z],0.00);
        // Caractéristiques du texte: couleur, limite, fonte, taille, caractères
  ...
  begin
    ...
    try
      ...
      for i:=0 to self.Count -1 do
        begin
            AN:=GetAnnotation(i);
            r:=GetRValue(AN.FontColor);
            g:=GetGValue(AN.FontColor);
            b:=GetBValue(AN.FontColor);
