...
begin
  ...
  try
    ...
    if hDLL=0 then begin
      ShowMessage(LIB_DECLIMAG + ': Librairie inopérante');
      Exit;
    end;
    // routine de démarrage
    @pProcDeclimagInitialise := GetProcAddress(hDLL, 'DLLCalcDeclimagInitialise');
    if @pProcDeclimagInitialise=nil then begin
      ShowMessage('DLLCalcDeclimagInitialise() inoperant');
      FreeLibrary(hDLL);
      Exit;
    end;
    // routine de libération
    @pProcDeclimagFinalise :=  GetProcAddress(hDLL, 'DLLCalcDeclimagFinalise');
    if @pProcDeclimagFinalise = nil then begin
      ShowMessage('DLLCalcDeclimagFinalise() inoperant');
      FreeLibrary(hDLL);
      Exit;
    end;
    // recherche des routines de la DLL
    @pProcHello:=GetProcAddress(hDll,'HelloWorld');
    if @pProcHello=nil then begin
      ShowMessage('HelloWorld() inoperant');
      FreeLibrary(hDLL);
      Exit;
    end;
    @pProcDLLHypot:=GetProcAddress(hDll,'DLLHypotenuse');
    if @pProcDLLHypot=nil then begin
      ShowMessage('DLLHypotenuse() inoperant');
      FreeLibrary(hDLL);
      Exit;
    end;
    @pProcCalcDeclimag := GetProcAddress(hDLL, 'DLLCalcDeclimag');
    if @pProcCalcDeclimag=nil then begin
      ShowMessage('DLLCalcDeclimag() inoperant');
      FreeLibrary(hDLL);
      Exit;
    end;
    Result := True;
